...
def _extract_metrics(image: np.ndarray, pose_landmarks) -> BodyMetrics:
    h, w = image.shape[:2]
    ...
    torso_lean = definition / max(waist_est, 0.01)
...
def analyze_single_image(b64: str) -> tuple[BodyMetrics | None, bool]:
    ...
    if not result.pose_landmarks:
        return None, False
    metrics = _extract_metrics(image, result.pose_landmarks)
    return metrics, True
...
def compare_before_after(
    ...
    for region_id, w in sorted(weights.items(), key=lambda x: -x[1]):
        ...
        weight_sum += w
        primary_count += 1
        if status == "improved" or (status == "maintained" and avg_score >= 50):
            aligned_count += 1
