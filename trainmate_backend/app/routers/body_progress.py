...
@router.post("/body-comparison", response_model=BodyComparisonOut)
def body_comparison(
    ...
    if not before and plan and plan.before_photo_base64:
        before = plan.before_photo_base64.strip()
    if not after and plan and plan.after_photo_base64:
        after = plan.after_photo_base64.strip()
    ...
    category = (body.category or (plan.category if plan else None) or "Muscle Gain").strip()
    template = (body.template_category or "").strip() or category
